/** 
☑️ 资源解析器 ©𝐒𝐡𝐚𝐰𝐧  ⟦2022-08-23 14:15⟧
----------------------------------------------------------
...
var para1 = para
  ...
  .replace(/\$index/g, "node_index_prefix"); //防止参数中其它位置也存在"#"
...
var Pin0 =
  mark0 && para1.indexOf("in=") != -1
    ...
    : null;
var Pout0 =
  mark0 && para1.indexOf("out=") != -1
    ? para1.split("out=")[1].split("&")[0].split("+").map(decodeURIComponent)
    : null;
var Psfilter =
...
var Preg =
  mark0 && para1.indexOf("regex=") != -1
    ? decodeURIComponent(para1.split("regex=")[1].split("&")[0]).replace(/\，/g, ",")
    : null; //server正则过滤参数
var Pregout =
  mark0 && para1.indexOf("regout=") != -1
    ? decodeURIComponent(para1.split("regout=")[1].split("&")[0]).replace(/\，/g, ",")
    : null; //server正则删除参数
var Pregdel =
  mark0 && para1.indexOf("delreg=") != -1
    ? decodeURIComponent(para1.split("delreg=")[1].split("&")[0]).replace(/\，/g, ",")
    : null; // 正则删除参数
var Phin0 =
...
var Phout0 =
  mark0 && para1.indexOf("outhn=") != -1
    ...
    : null; //hostname
var Preplace =
  mark0 && para1.indexOf("replace=") != -1 ? para1.split("replace=")[1].split("&")[0] : null; //filter/rewrite 正则替换
...
var PProfile =
  mark0 && para1.indexOf("profile=") != -1 ? para1.split("profile=")[1].split("&")[0] : 0; // 通过URL-Scheme导入完整配置参数
var Palpn =
...
function ResourceParse() {
  ...
  if (flag == 1) {
    ...
    if (Preplace) {
      // server 类型也可用 replace 参数进行重命名操作
      total = ReplaceReg(total, Preplace);
    }
    if (Prname) {
      Prn = Prname;
    ...
    if (total.length > 0) {
      ...
      if (Pflow == 1) {
        ...
        $done({
          content: total,
          info: { bytes_used: 3073741824, bytes_remaining: 2147483648, expire_date: 1854193966 }
        });
        //$notify("done?","strange")
      } else {
        $done({ content: total });
...
function SubFlow() {
  if (Pinfo == 1 && subinfo) {
    ...
    var total =
      "总流量: " +
      (parseFloat(sinfo.split("total=")[1].split(",")[0]) / 1024 ** 3).toFixed(2) +
      "GB";
    var usd =
      "已用流量: " +
      (
...
function Type_Check(subs) {
  ...
  if (
    ...
    (/^hostname\s*\=|pattern\=/.test(subi) || RewriteK.some(RewriteCheck)) &&
    ...
    // Surge 类型 module /rule-set(含url-regex) 类型
    typec = "rewrite";
    ...
    // 仅限用户指定为 domain-set；((DomainK.some(RuleCheck) || typeU == "domain-set") && subs.indexOf("[Proxy]") == -1 && typeU != "nodes") {
    typec = "filter";
    type = typeQ == "unsupported" || typeQ == "filter" ? "Rule" : "wrong-field";
    content0 = Domain2Rule(content0); // 转换 domain-set
  } else if (typeQ == "filter") {
    // 纯 list类型？
    typec = "filter";
    type = typeQ == "unsupported" || typeQ == "filter" ? "Rule" : "wrong-field";
  } else if (
    ...
    typec = "server";
    type = typeQ == "unsupported" || typeQ == "server" || typeQ == "uri" ? "Subs" : "wrong-field";
  } else if (content0.indexOf("server") != -1 && content0.indexOf("server_port") != -1) {
    //SIP008
    //type = "QuanX"
    typec = "server";
...
function TagCheck_QX(content) {
  ...
  for (var i = 0; i < Olist.length; i++) {
    ...
    typefix["shadowsocks"] =
      item.indexOf("ssr-protocol") != -1 ? typefix["shadowsocksr"] : typefix["shadowsocks"];
...
function type_prefix(item) {
  if (item.trim() != "") {
    ...
    return [[item.split("tag=")[0] + "tag=", tp, item.split("tag=")[1]].join(" ")].join(" ");
  }
}
...
function HOST_Handle(cnt, phost) {
  ...
  if (phost.indexOf("☠️") == -1) {
    ...
    // 为已有的替换，为没有的增加 obfs-host\tls-host
...
function SHA256_Handle(cnt, pcsha256, ppsha256) {
  if (cnt.indexOf("over-tls=true") != -1 || cnt.indexOf("obfs=wss") != -1) {
    ...
    cnt =
      ppsha256 != "" ? cnt.replace(/tag\s*\=/, "tls-pubkey-sha256=" + ppsha256 + ", tag=") : cnt;
  }
  return cnt;
}
  ...
  for (var i = 0; i < subs.length; i++) {
    ...
    if (!NoteK.some(notecheck)) {
      if (subs[i].slice(0, 9) == "URL-REGEX") {
        // regex 类型
        rw =
          subs[i].replace(/ /g, "").split(",REJECT")[0].split("GEX,")[1] + " url " + "reject-200";
        nrw.push(rw);
  ...
  if (Pntf0 != 0) {
    ...
    if (Pin0 && no1write != " 0️⃣ ") {
      ...
      $notify(
        "🤖 " + "重写引用  ➟ " + "⟦" + subtag + "⟧",
        "⛔️ 筛选参数: " + pfi + pfo,
        "☠️ 重写 rewrite 中保留以下" + no1write + "个匹配项:" + "\n ⨷ " + Nlist.join("\n ⨷ "),
        rwrite_link
      );
      ...
      $notify(
        "🤖 " + "重写引用  ➟ " + "⟦" + subtag + "⟧",
        "⛔️ 筛选参数: " + pfi + pfo,
        "☠️ 重写 rewrite 中已禁用以下" + nowrite + "个匹配项:" + "\n ⨷ " + dwrite.join("\n ⨷ "),
        rwrite_link
      );
    }
  }
...
function Rule_Handle(subs, Pout, Pin) {
  ...
  if (Tout != "" && Tout != null) {
    ...
    for (var i = 0; i < cnt.length; i++) {
      ...
      } else if (!RuleK.some(CommentCheck) && cc && RuleK2.some(RuleCheck)) {
        //if Pout.some, 不操作注释项，不操作不识别规则项目
        dd = Rule_Policy(cc);
        if (Tin != "" && Tin != null) {
    ...
    if (list0[i].trim().length > 3 && !/\;|\/|\#/.test(list0[i][0])) {
      ...
      var tag0 =
        list0[i].indexOf("tag=") != -1
          ? list0[i].split(/\&*(emoji|udp|tfo|cert|rename|replace)\=/)[0].split("tag=")[1]
          : "";
...
function TLS_Check(cnt) {
  cnt =
    cnt.indexOf("tls-cert-sha256") != -1 || cnt.indexOf("tls-pubkey-sha256") != -1
      ? cnt.replace(/tls-verification\s*\=\s*false.*?\,/, "tls-verification=true,")
      : cnt; // 去掉 tls-verification=false 如果存在 sha256
  return cnt;
}
...
function QX_TLS(cnt, Pcert0, PTls13) {
  ...
  if (cnt.indexOf("tls-verification") != -1) {
    // 已有tls参数时, 如用户不指定，则不做处理
    cnt =
      Pcert0 == -1 || Pcert0 == 1 ? cnt.replace(RegExp("tls-verification.*?,", "gmi"), cert0) : cnt;
  ...
  if (!/^(shadowsocks|trojan|vmess)/.test(cnt.trim())) {
    //关闭非 ss/ssr/trojan/vmess 类型的 udp
    udp = "udp-relay=false, ";
  ...
  if (server.indexOf("obfs=") == -1) {
    ...
    var host =
      server.indexOf("obfs-header=") != -1
        ? "obfs-host=" + server.split('obfs-header="Host:')[1].split("[")[0].trim() + ", "
        : "";
    obfs = obfs + host;
...
function VR2QX(subs, Pudp, Ptfo, Pcert0, PTls13) {
  var server = String(
    Base64.decode(
      subs.replace("vmess://", "").split("?remark")[0].split("&remark")[0].split("?")[0]
    ).trim()
  ).split("\u0000")[0];
  var node = "";
  var ip =
  ...
  if (obfs == "none") {
    ...
    var host =
      subs.indexOf("&obfsParam=") != -1
        ? decodeURIComponent(subs.split("&obfsParam=")[1].split("&")[0].split("\n")[0])
            .split("\n")[0]
            .trim()
        : "";
...
function SSR2QX(subs, Pudp, Ptfo) {
  var nssr = [];
  var cnt = Base64.decode(subs.split("ssr://")[1].replace(/-/g, "+").replace(/_/g, "/")).split(
    "\u0000"
  )[0];
  var obfshost = "";
  var oparam = "";
  if (cnt.split(":").length <= 8) {
    ...
    pwd =
      "password=" +
      Base64.decode(cnt.split("/?")[0].split(":")[5].replace(/-/g, "+").replace(/_/g, "/")).split(
        "\u0000"
      )[0];
    ...
    if (cnt.indexOf("obfsparam=") != -1) {
      obfshost =
        cnt.split("obfsparam=")[1].split("&")[0] != ""
          ? "obfs-host=" +
            Base64.decode(
              cnt.split("obfsparam=")[1].split("&")[0].replace(/-/g, "+").replace(/_/g, "/")
            )
              .split(",")[0]
              .split("\u0000")[0]
          : "";
    ...
    if (cnt.indexOf("protoparam=") != -1) {
      oparam =
        cnt.split("protoparam=")[1].split("&")[0] != ""
          ? "ssr-protocol-param=" +
            Base64.decode(
              cnt.split("protoparam=")[1].split("&")[0].replace(/-/g, "+").replace(/_/g, "/")
            )
              .split(",")[0]
              .split("\u0000")[0]
          : "";
    ...
    tag =
      "tag=" +
      Base64.decode(
        cnt.split("remarks=")[1].split("&")[0].replace(/-/g, "+").replace(/_/g, "/")
      ).split("\u0000")[0];
  ...
  if (cnt.indexOf("&plugin=obfs-local") != -1) {
    ...
    thost =
      cnt.indexOf("obfs-host=") == -1
        ? thost
        : "obfs-host=" + decodeURIComponent(cnt.split("obfs-host=")[1].split(";")[0].split("#")[0]);
    puri =
      cnt.indexOf("obfs-uri=") == -1
        ? puri
        : ", obfs-uri=" + decodeURIComponent(cnt.split("obfs-uri=")[1].split(";")[0].split("#")[0]);
    ...
    thost =
      cnt.indexOf("&host=") == -1
        ? thost
        : "obfs-host=" + decodeURIComponent(cnt.split("&host=")[1].split("&")[0].split("#")[0]);
    puri =
      cnt.indexOf("&path=") == -1
        ? puri
        : ", obfs-uri=" + decodeURIComponent(cnt.split("&path=")[1].split("&")[0].split("#")[0]);
  }
...
function SS2QX(subs, Pudp, Ptfo) {
  ...
  if (cnt.split(":").length <= 6) {
    ...
    if (cntt.indexOf("@") != -1 && cntt.indexOf(":") != -1) {
      ...
      pwdmtd = Base64.decode(cnt.split("@")[0].replace(/-/g, "+").replace(/_/g, "/"))
        .split("\u0000")[0]
        .split(":");
    } else if (cntt.indexOf("?") == -1) {
      // 后部 b64 encode 类型
      var cnt0 = Base64.decode(
        cnt.split("#")[0].replace(/-/g, "+").replace(/_/g, "/").split("\u0000")[0]
      );
      ...
      // 火箭类型？
      var cnt0 = Base64.decode(
        cnt.split("#")[0].split("?")[0].replace(/-/g, "+").replace(/_/g, "/").split("\u0000")[0]
      );
    ...
    if (cntt.indexOf("v2ray-plugin") == -1) {
      //Shadowrocket style v2-plugin
      obfs =
      ...
      obfshost =
        cnt.split("obfs-host%3D")[1] != null
          ? ", obfs-host=" + cnt.split("obfs-host%3D")[1].split("&")[0].split("#")[0]
          : "";
    } else {
...
function SSD2QX(subs, Pudp, Ptfo) {
  ...
  if (cnt.plugin_options) {
    ...
    obfshost =
      cnt.plugin_options.split(";")[1] != null ? ", " + cnt.plugin_options.split(";")[1] : "";
  }
  pudp = Pudp == 1 ? "udp-relay=true" : "udp-relay=false";
  ptfo = Ptfo == 1 ? "fast-open=true" : "fast-open=false";
  ...
  for (var i = 0; i < cnt.length; i++) {
    if (cnt[i]) {
      ...
      if (
        ...
        (cnti.indexOf(" 302") != -1 || cnti.indexOf(" 307") != -1) &&
        cnti.indexOf(" url ") == -1
      ) {
        cnti = SGMD2QX(cnti)[0] ? SGMD2QX(cnti)[0] : "";
...
function Rename(str) {
  ...
  if (server.indexOf("tag=") != -1) {
    ...
    tail =
      server.split("tag=")[1].split(",").length <= 1 ? "" : server.split("tag=")[1].split(name)[1];
  ...
  for (var key in cnt) {
    ...
    for (i in dd) {
      if (sname.indexOf(dd[i]) != -1) {
        flag = 1;
        nname =
          key + " " + sname.replace(/[\uD83C][\uDDE6-\uDDFF][\uD83C][\uDDE6-\uDDFF]/g, "").trim(); // use regex to remove the original flag
        return [nname, key];
      }
    }
  }
...
function SVmess2QX(content) {
  ...
  var puname =
    cnt.indexOf("username") != -1
      ? "password=" + cnt.split("username")[1].split(",")[0].split("=")[1].trim()
      : "";
  ...
  var phost =
    cnt.indexOf("ws-headers") != -1
      ? "obfs-host=" + cnt.split("ws-headers")[1].split(",")[0].split("=")[1].split(":")[1].trim()
      : "";
...
function Strojan2QX(content) {
  ...
  var pverify =
    cnt.replace(/ /g, "").indexOf("skip-cert-verify=false") != -1
      ...
      : "tls-verification=false";
  var phost =
    cnt.indexOf("sni") != -1 ? "tls-host=" + cnt.split("sni")[1].split(",")[0].split("=")[1] : "";
  pvefify = Pcert0 == 1 ? "tls-verification=true" : pverify;
  var ptls13 = paraCheck(cnt, "tls13") == "true" ? "tls13=true" : "tls13=false";
  var nserver =
    "trojan= " + [ipport, pwd, ptls, ptfo, ptls13, phost, pverify, tag].filter(Boolean).join(", ");
  return nserver;
}
// surge 中的 http 类型
function Shttp2QX(content) {
  ...
  var puname =
    cnt.indexOf("username") != -1
      ? "username=" + cnt.split("username")[1].split(",")[0].split("=")[1].trim()
      : "";
  var pwd =
...
function SS52QX(content) {
  ...
  var pwd =
    cnt.indexOf("password") != -1
      ...
      : "";
  var ptls =
    cnt.split("=")[1].split(",")[0].trim() == "socks5-tls" ? "over-tls=true" : "over-tls=false";
  var ptfo = paraCheck(cnt, "tfo") == "true" ? "fast-open=true" : "fast-open=false";
  if (ptls == "over-tls=true") {
    var pverify =
      cnt.replace(/ /g, "").indexOf("skip-cert-verify=false") != -1
...
function LoonSS2QX(cnt) {
  ...
  var obfs =
    cnt.split(",").length == 7
      ? ", " +
        ["obfs=" + cnt.split(",")[5].trim(), "obfs-host=" + cnt.split(",")[6].trim()].join(",")
      : "";
  var tag = ", tag=" + cnt.split("=")[0].trim();
...
function CSS2QX(cnt) {
  ...
  node =
    "shadowsocks=" +
    [ipt, pwd, mtd, udp, tfo, obfs, ohost, ouri, cert, tag].filter(Boolean).join(", ");
  return node;
}
...
function CSSR2QX(cnt) {
  ...
  node =
    "shadowsocks=" +
    [ipt, pwd, mtd, udp, tfo, prot, ppara, obfs, ohost, tag].filter(Boolean).join(", ");
  //console.log(node)
  return node;
}
...
function CV2QX(cnt) {
  ...
  node =
    "vmess=" +
    [ipt, pwd, mtd, udp, tfo, obfs, ohost, ouri, cert, caead, tag].filter(Boolean).join(", ");
  //console.log(node)
  return node;
}
...
function CT2QX(cnt) {
  ...
  node =
    "trojan=" +
    [ipt, pwd, otls, opath, ohost, cert, tls13, udp, tfo, tag].filter(Boolean).join(", ");
  //console.log(node)
  return node;
}
