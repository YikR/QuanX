...
[Script]
QX转换 = type=http-request,pattern=qx$,requires-body=1,max-size=0,script-path=qx2surge.js
      ...
      switch (type) {
        case "script-":
          ...
          script.push(
            x.replace(
              ...
              `$4 = type=http-$2,pattern=$1,requires-body=1${proto},max-size=0,script-path=$3,script-update-interval=-1`
            )
          );
          break;
        ...
        case "reject":
          ...
          if (jct === "reject-img") {
            z[y - 1]?.match("#") && MapLocal.push(z[y - 1]);
            MapLocal.push(
              `${url} data="https://raw.githubusercontent.com/mieqq/mieqq/master/reject-img.gif"`
            );
            break;
          }
